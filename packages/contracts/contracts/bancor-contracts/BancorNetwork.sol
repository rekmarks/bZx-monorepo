...
contract BancorNetwork is IBancorNetwork, TokenHolder, ContractIds, FeatureIds {
    ...
    address public signerAddress = address(0x0);         // verified address that allows conversions with higher gas price
    ...
    */
    ...
    {
        etherTokens[address(_token)] = _register;
    }
    ...
    */
    function verifyTrustedSender(IERC20Token[] memory _path, uint256 _amount, uint256 _block, address _addr, uint8 _v, bytes32 _r, bytes32 _s) private returns(bool) {
    ...
    */
    ...
    /**
        ...
        @return tokens issued in return
    */
    function convertForPrioritized2(IERC20Token[] memory _path, uint256 _amount, uint256 _minReturn, address payable _for, uint256 _block, uint8 _v, bytes32 _r, bytes32 _s)
        public
        payable
        ...
        returns (uint256)
    {
        ...
        require(msg.value == 0 || (_amount == msg.value && etherTokens[address(fromToken)]));
        ...
        if (msg.value > 0)
            IEtherToken(address(fromToken)).deposit.value(msg.value)();
    ...
    */
    function convertForMultiple(IERC20Token[] memory _paths, uint256[] memory _pathStartIndex, uint256[] memory _amounts, uint256[] memory _minReturns, address payable _for)
        public
        payable
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < _pathStartIndex.length; i += 1) {
            ...
            require(msg.value == 0 || (_amounts[i] <= msg.value && etherTokens[address(fromToken)]) || !etherTokens[address(fromToken)]);

            // if ETH was sent with the call, the source is an ether token - deposit the ETH path amount in it.
            // otherwise, we assume we already have the tokens
            if (msg.value > 0 && etherTokens[address(fromToken)]) {
                IEtherToken(address(fromToken)).deposit.value(_amounts[i])();
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if (etherTokens[address(toToken)])
            IEtherToken(address(toToken)).withdrawTo(_for, _amount);
        else
    ...
    */
    function convertByPath(
        ...
        for (uint256 i = 1; i < pathLength; i += 2) {
            smartToken = ISmartToken(address(_path[i]));
            toToken = _path[i + 1];
            ...
            if (smartToken != _fromToken)
                ensureAllowance(_fromToken, address(converter), _amount);
    ...
    */
    function checkWhitelist(IBancorConverter _converter, address _for, IContractFeatures _features) private view {
        ...
        if (!_features.isSupported(address(_converter), FeatureIds.CONVERTER_CONVERSION_WHITELIST))
            return;
    ...
    /**
        ...
        @return tokens issued in return
    */
    function claimAndConvertFor(IERC20Token[] memory _path, uint256 _amount, uint256 _minReturn, address payable _for) public returns (uint256) {
        ...
        IERC20Token fromToken = _path[0];
        assert(fromToken.transferFrom(msg.sender, address(this), _amount));
    ...
    */
        ...
        returns(uint32)
    {
        ...
        bool isSet;
        (virtualBalance, weight, isVirtualBalanceEnabled, isPurchaseEnabled, isSet) = _converter.connectors(address(_connector));
        return weight;
    }
