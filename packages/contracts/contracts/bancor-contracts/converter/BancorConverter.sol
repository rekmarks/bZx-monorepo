...
contract BancorConverter is IBancorConverter, SmartTokenController, Managed, ContractIds, FeatureIds {
    ...
    mapping (address => Connector) internal connectors_;   // connector token addresses -> connector data
    ...
    */
    ...
    modifier validConversionPath(IERC20Token[] memory _path) {
    ...
    */
    function withdrawTokens(IERC20Token _token, address _to, uint256 _amount) public {
        ...
        require(!connectors_[address(_token)].isSet || token.owner() != address(this) || owner == converterUpgrader);
    ...
    */
    ...
    {
        require(_token != token && !connectors_[address(_token)].isSet && totalConnectorWeight + _weight <= MAX_WEIGHT); // validate input

        connectors_[address(_token)].virtualBalance = 0;
        connectors_[address(_token)].weight = _weight;
        connectors_[address(_token)].isVirtualBalanceEnabled = _enableVirtualBalance;
        connectors_[address(_token)].isPurchaseEnabled = true;
        connectors_[address(_token)].isSet = true;
    ...
    */
    function updateConnector(IERC20Token _connectorToken, uint32 _weight, bool _enableVirtualBalance, uint256 _virtualBalance)
        ...
        validConnectorWeight(_weight)
    {
        Connector storage connector = connectors_[address(_connectorToken)];
    ...
    */
    function disableConnectorPurchases(IERC20Token _connectorToken, bool _disable)
        ...
        validConnector(_connectorToken)
    {
        connectors_[address(_connectorToken)].isPurchaseEnabled = !_disable;
    }
    ...
    */
    ...
    {
        Connector storage connector = connectors_[address(_connectorToken)];
        return connector.isVirtualBalanceEnabled ? connector.virtualBalance : _connectorToken.balanceOf(address(this));
    }
    ...
    */
    function getCrossConnectorReturn(IERC20Token _fromConnectorToken, IERC20Token _toConnectorToken, uint256 _sellAmount)
        ...
        returns (uint256, uint256)
    {
        Connector storage fromConnector = connectors_[address(_fromConnectorToken)];
        Connector storage toConnector = connectors_[address(_toConnectorToken)];
    ...
    */
        ...
        returns (uint256)
    {
        ...
        Connector storage fromConnector = connectors_[address(_fromToken)];
        ...
        Connector storage toConnector = connectors_[address(_toToken)];
        ...
        assert(_fromToken.transferFrom(msg.sender, address(this), _amount));
        ...
        emit PriceDataUpdate(address(_fromToken), token.totalSupply(), getConnectorBalance(_fromToken), fromConnector.weight);
        emit PriceDataUpdate(address(_toToken), token.totalSupply(), getConnectorBalance(_toToken), toConnector.weight);
        return amount;
    }
    ...
    */
    function buy(IERC20Token _connectorToken, uint256 _depositAmount, uint256 _minReturn) internal returns (uint256) {
        ...
        assert(_connectorToken.transferFrom(msg.sender, address(this), _depositAmount));
        ...
        emit PriceDataUpdate(address(_connectorToken), token.totalSupply(), getConnectorBalance(_connectorToken), connector.weight);
        return amount;
    }
    ...
    */
    function quickConvert(IERC20Token[] memory _path, uint256 _amount, uint256 _minReturn)
        public
        payable
    ...
    /**
        ...
        @return tokens issued in return
    */
    function quickConvertPrioritized(IERC20Token[] memory _path, uint256 _amount, uint256 _minReturn, uint256 _block, uint8 _v, bytes32 _r, bytes32 _s)
        public
        payable
        validConversionPath(_path)
        returns (uint256)
    {
        ...
        if (msg.value == 0) {
            ...
            if (fromToken == token) {
                ...
                token.issue(address(bancorNetwork), _amount); // issue _amount new tokens to the BancorNetwork contract
            } else {
                // otherwise, we assume we already have allowance, transfer the tokens directly to the BancorNetwork contract
                assert(fromToken.transferFrom(msg.sender, address(bancorNetwork), _amount));
            }
        }
    ...
    */
        ...
        conversionsAllowed
    {
        ...
        for (uint16 i = 0; i < connectorTokens.length; i++) {
            ...
            // update virtual balance if relevant
            Connector storage connector = connectors_[address(connectorToken)];
            if (connector.isVirtualBalanceEnabled)
                connector.virtualBalance = safeAdd(connector.virtualBalance, connectorAmount);

            // transfer funds from the caller in the connector token
            assert(connectorToken.transferFrom(msg.sender, address(this), connectorAmount));

            // dispatch price data update for the smart token/connector
            emit PriceDataUpdate(address(connectorToken), supply + _amount, connectorBalance + connectorAmount, connector.weight);
        }
    ...
    */
    function dispatchConversionEvent(IERC20Token _fromToken, IERC20Token _toToken, uint256 _amount, uint256 _returnAmount, uint256 _feeAmount) private {
        ...
        emit Conversion(address(_fromToken), address(_toToken), msg.sender, _amount, _returnAmount, int256(_feeAmount));
    }
}
