...
contract BancorConverterUpgrader is IBancorConverterUpgrader, Owned, ContractIds, FeatureIds {
    ...
    */
    function upgradeOld(IBancorConverter _converter, bytes32 _version) public {
        ...
        if (_version == "0.4")
            formerVersions = true;
        IBancorConverterExtended converter = IBancorConverterExtended(address(_converter));
    ...
    */
    function acceptConverterOwnership(IBancorConverterExtended _oldConverter) private {
        ...
        emit ConverterOwned(address(_oldConverter), address(this));
    }
    ...
    */
    function createConverter(IBancorConverterExtended _oldConverter) private returns(IBancorConverterExtended) {
        ...
        if (features.isSupported(address(_oldConverter), FeatureIds.CONVERTER_CONVERSION_WHITELIST)) {
    ...
    */
        ...
        private
    {
        ...
        for (uint16 i = 0; i < connectorTokenCount; i++) {
            address connectorAddress = _isLegacyVersion ? address(_oldConverter.reserveTokens(i)) : address(_oldConverter.connectorTokens(i));
    ...
    */
        ...
        private
    {
        ...
        for (uint16 i = 0; i < connectorTokenCount; i++) {
            ...
            connectorBalance = connector.balanceOf(address(_oldConverter));
